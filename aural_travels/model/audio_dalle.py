...
class AudioDALLE(nn.Module):
    ...
    def forward(self, audio_emb, image):
        assert audio_emb.shape[1] == self.audio_seq_len

        audio_emb = audio_emb \
            + self.audio_pos_emb(torch.arange(audio_emb.shape[1], device=audio_emb.device))

        image_emb = self.image_emb(image)
        image_emb = image_emb + self.image_pos_emb(image_emb)
